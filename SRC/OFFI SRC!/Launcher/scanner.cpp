...
BACK:
	...
	// 숫자
	if( isdigit2( *pProg ) && !IsDBCSLeadByte( *pProg ) ) //숫자 검사만으로 끝나는데 왜 한글이 아닐 경우도 체크하지?
	{
		while( !isdelim( *pProg ) ) *pszTemp++ = *pProg++;
		tokenType = NUMBER; goto EXIT;
	}
	// 변수와 명령
	if( isalpha( *pProg ) || IsDBCSLeadByte( *pProg ) || *pProg == '#' || *pProg == '_' || *pProg == '@' || *pProg=='$' || *pProg == '?' ) 
	{
		while(!isdelim(*pProg)) *pszTemp++ = *pProg++;
	}
	else
