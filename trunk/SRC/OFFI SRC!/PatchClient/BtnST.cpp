...
DWORD CButtonST::SetBitmaps(char* filename_in, COLORREF crTransColorIn, char* filename_out, COLORREF crTransColorOut)
{
	HBITMAP		hBitmapIn		= NULL;
	HBITMAP		hBitmapOut		= NULL;
	HINSTANCE	hInstResource	= NULL;
	...
	// Find correct resource handle
	hInstResource = AfxGetInstanceHandle();//AfxFindResourceHandle(MAKEINTRESOURCE(nBitmapIn), RT_BITMAP);
	
	// Load bitmap In
	hBitmapIn = (HBITMAP)::LoadImage(hInstResource, filename_in, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION | LR_DEFAULTSIZE);
	
	// Load bitmap Out
	if (filename_out)
		hBitmapOut = (HBITMAP)::LoadImage(hInstResource, filename_out, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION | LR_DEFAULTSIZE);
...
DWORD CButtonST::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
	...
	if( !m_bBmp )
	{
		m_csMyBitmaps[0] = hBitmapIn;
		m_csMyBitmaps[1] = hBitmapOut;
...
void CButtonST::EnableButton(BOOL visible)
{
//	m_bIsDisabled = visible;

//	EnableWindow(visible);	
	
	//if( visible )
	//	SetBitmaps(m_csMyBitmaps[0], m_csBitmaps[0].crTransparent, m_csMyBitmaps[1], m_csBitmaps[1].crTransparent );//, m_csMyBitmaps[2], m_csBitmaps[2].crTransparent );
	//else
	//	SetBitmaps(m_csMyBitmaps[0], m_csBitmaps[0].crTransparent );
	//if( !visible )
	//	SetBitmaps(m_csMyBitmaps[0], m_csBitmaps[0].crTransparent );	
}
