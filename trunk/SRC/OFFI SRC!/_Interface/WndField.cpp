...
#endif //__Y_NEW_ENCHANT
				{
					...
					else if(pItemMaterialElem->m_dwItemId == II_GEN_MAT_MOONSTONE || pItemMaterialElem->m_dwItemId == II_GEN_MAT_MOONSTONE_1)
					{
						...
						if( pItemElem->IsCollector( TRUE ) || pItemElem->GetProp()->dwItemKind2 == IK2_JEWELRY )
						{
							...
							{
								g_WndMng.PutString( "뚤꼇폅，퀭돨踏狗귑꼇璃。", NULL, prj.GetTextColor( TID_GAME_NOTCOUPLETARGET ) );
								return false;
							}
	...
	if( m_bIsUpgradeMode )
	{
		...
		else if( IsNeedTarget( pItemProp ) )
		{
			...
			if(g_WndMng.m_pWndNew)
			{
				...
				CString strTitle;
				if(pItemProp2->dwItemKind2 == IK2_CLOTH || pItemProp2->dwItemKind2 == IK2_CLOTHETC )
					strTitle.Format("굶늴뻥今矜狼句봬100,000踏狗귑，횅땍역迦찐？");
				else
					strTitle.Format("굶늴뻥今矜狼句봬50,000踏狗귑，횅땍역迦찐？");
			...
			if(m_pUpgradeMaterialItem->m_dwItemId == II_GEN_MAT_MOONSTONE || m_pUpgradeMaterialItem->m_dwItemId == II_GEN_MAT_MOONSTONE_1)
			{
				...
				if( ( static_cast<CItemElem*>( pItem ) )->IsCollector( TRUE ) || pItem->GetProp()->dwItemKind2 == IK2_JEWELRY )
				{
					...
					if(g_WndMng.m_pWndNew)
					{
						...
						DWORD dwCost = prj.GetEnchantCost(nAbilityOption);
						CString strTitle;
						strTitle.Format("굶늴쑹조矜狼句봬%u踏狗귑，횅땍역迦찐？",dwCost);
						g_WndMng.m_pWndNew->m_pStatic->SetTitle(strTitle);
					}
					return;
				}
		...
		if(g_WndMng.m_pWndNew)
		{
			...
			DWORD dwCost = prj.GetEnchantCost(nAbilityOption);
			CString strTitle;
			strTitle.Format("굶늴쑹조矜狼句봬%u踏狗귑，횅땍역迦쑹조찐？",dwCost);
			g_WndMng.m_pWndNew->m_pStatic->SetTitle(strTitle);
		}
	}
}
				...
				if(nID == IDOK)
				{
					if(m_pTargetObj)
					{
						if(g_pPlayer->GetGold()<3000000)
							g_WndMng.PutString( "뚤꼇폅，퀭돨踏狗귑꼇璃，轟랬랙폅멩겜。", NULL, prj.GetTextColor( TID_GAME_NOTCOUPLETARGET ) );
						else
						{
