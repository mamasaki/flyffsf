...
void CWndVIP::OnDraw( C2DRender* p2DRender ) 
{ 
} 
void CWndVIP::OnInitialUpdate() 
{ 
	CWndNeuz::OnInitialUpdate(); 
	// 여기에 코딩하세요

	CWndText* pWndText = (CWndText*)GetDlgItem( WIDC_TEXT1 );
	CWndText* pWndUSE_0 = (CWndText*)GetDlgItem( WIDC_USE_0 );
	CWndText* pWndUSE_1 = (CWndText*)GetDlgItem( WIDC_USE_1 );
	CWndText* pWndUSE_2 = (CWndText*)GetDlgItem( WIDC_USE_2 );
	...
	if( CFile::GetStatus( "Client\\VIP.inc", fileStatus ) == TRUE )
	{
		...
		//else
		{
			if( file.Open( "Client\\VIP.inc", "rb" ) )
			{
				int nLength = file.GetLength();
...
void CWndVIP::AfterSkinTexture( LPWORD pDest, CSize size, D3DFORMAT d3dFormat )
{
	CPoint pt;
	LPWNDCTRL lpWndCtrl;
...
BOOL CWndVIP::Initialize( CWndBase* pWndParent, DWORD /*dwWndId*/ ) 
{ 
	// Daisy에서 설정한 리소스로 윈도를 연다.
	return CWndNeuz::InitDialog( g_Neuz.GetSafeHwnd(), APP_INFO_VIP, 0, CPoint( 0, 0 ), pWndParent );
} 
BOOL CWndVIP::OnCommand( UINT nID, DWORD dwMessage, CWndBase* pWndBase ) 
{ 
	return CWndNeuz::OnCommand( nID, dwMessage, pWndBase ); 
} 
void CWndVIP::OnSize( UINT nType, int cx, int cy ) \
{ 
	CWndNeuz::OnSize( nType, cx, cy ); 
} 
void CWndVIP::OnLButtonUp( UINT nFlags, CPoint point ) 
{ 
} 
BOOL CWndVIP::OnSetCursor( CWndBase* pWndBase, UINT nHitTest, UINT message )
{
...
void CWndVIP::OnLButtonDown( UINT nFlags, CPoint point ) 
{ 
	if( ::GetLanguage() == LANG_KOR )
	{
		LPWNDCTRL pCustom = NULL;
...
BOOL CWndVIP::OnChildNotify( UINT message, UINT nID, LRESULT* pLResult ) 
{ 
	switch( nID )
	{
	case WIDC_CLOSE:
