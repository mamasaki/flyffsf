...
void CDPSrvr::OnRepairItem( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		for(int  j = 0; j < c; j++ )
		{
			if( apItemRepair[j] )
			{
				pUser->UpdateItem( (BYTE)apItemRepair[j]->m_dwObjId, UI_HP, apItemRepair[j]->GetProp()->dwEndurance );
				pUser->UpdateItem( (BYTE)apItemRepair[j]->m_dwObjId, UI_RN, apItemRepair[j]->m_nRepairNumber + anRepair[j] );
			}
		}
...
void	CDPSrvr::OnNPCBuff( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) ) 
	{
		...
		if( lpChar )
		{
			...
			{
				if( !CNpcChecker::GetInstance()->IsCloseNpc(MMI_NPC_BUFF3, pUser->GetWorld(), pUser->GetPos() ) )
					return;
				...
				if( pUser->GetLevel() >= vecNPCBuff[i].nMinPlayerLV && pUser->GetLevel() <= vecNPCBuff[i].nMaxPlayerLV )
				{
					...
					if( pSkillProp )
					{
						...
						if(	( pSkillProp->dwID == SI_GEN_EVE_QUICKSTEP && pUser->HasBuff(BUFF_SKILL, SI_ASS_CHEER_QUICKSTEP) )
							...
							|| ( pSkillProp->dwID == SI_GEN_ADDEXP_LV01 && pUser->HasBuff(BUFF_SKILL, SI_GEN_ADDEXP_LV02) )
							|| ( pSkillProp->dwID == SI_GEN_ADDDROP_LV01 && pUser->HasBuff(BUFF_SKILL, SI_GEN_ADDDROP_LV02) ))
						{
