...
BOOL CEnvironment::LoadScript()
{
	...
	while( scanner.tok != FINISHED )
	{
		...
		int nSubLang = scanner.GetNumber();
		if( nSubLang < 0 || nSubLang >= LANG_SUB_MAX )
		{
			Error( "CEnvironment::LoadScript - Invalid SubLang : %[d]", nSubLang );
			return FALSE;
		}
		...
		for( int i = 0; i < 12; ++i )
		{
			...
			m_aEnvironmentEffect[nLang][nSubLang][i] = nSeason;
		}
...
int CEnvironment::GetSeason()
{
	...
	if( nLang < 0 || nSubLang < 0 || nLang >= LANG_MAX || nSubLang >= LANG_SUB_MAX )
	{
		Error( "CEnvironment::GetSeason - Invalid argument. Lang : [%d], SubLang : [%d]", nLang, nSubLang );
		return SEASON_NONE;
	}
	...
	return m_aEnvironmentEffect[nLang][nSubLang][nMonth - 1];
}
#endif // __WORLDSERVER
